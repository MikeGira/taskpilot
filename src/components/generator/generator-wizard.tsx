...
function buildDownloadContent(result: GenerateResult): string {
  ...
  if (result.explanation) {
    ...
    for (const word of result.explanation.split(/(?<=\S{60,})|(?<=\s)/)) {
      const last = lines[lines.length - 1];
      if (last === `${c}` || last.length + word.length > 78) {
        lines.push(`${c}  ${word.trimStart()}`);
      } else {
